...
let hybrid intake_manifold (mdot_input, eng_speed) = (mdot_cyl, man_press)
  where
  rec der man_press = 0.41328 *. (mdot_input -. last mdot_cyl) init 0.589
...
let direction u = if u = 0.0 then 0.0 else if u > 0.0 then 1.0 else -. 1.0
...
let throttle (throt_angle, man_press, atm_press) = throt_flow where
  rec pratio = min (man_press /. atm_press, atm_press /. man_press)
  and throt_flow = f(throt_angle)
      *. (if pratio >= 0.5 then g(pratio) else sonic_flow) 
...
let hybrid throttle_and_manifold (engine_speed, throttle_angle) =
  ...
  rec throttle_flow =
    throttle (max (0.0, throttle_angle), last map, atmospheric_pressure)
                    (* TODO: this last is problematic! *)
...
let hybrid mixing (fuel_rate, air_flow) = (o2_out, air_fuel_ratio) where
  ...
  and air_fuel_ratio = statespace ((-. 3.0, 1.0, 3.0, 0.0), 0.02283105022831, y)
                       (* System Lag *)
