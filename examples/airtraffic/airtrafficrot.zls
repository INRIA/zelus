...
let hybrid avoidance (x_i, y_i, theta_i, v_1, v_2) = (st, x, y, theta, t) where
  ...
  and init st = 0
  and automaton
      | Start -> do t' = 0.0 until (init on true) then Cruise(x,y) done
      ...
      | Cruise(x0,y0) ->
          ...
          then TurningLeft(nx0, ny0) done

      | TurningLeft(x0, y0) ->
          local theta in
          do
                der theta = 10.0 init 0.0
            and t' = 0.0
            and (x, y) = rotate(-. theta, x0, y0)
            and st = 0
          until (up(theta -. delta_phi)) then Left(x, y)
          done

      | Left(x0,y0) ->
          local nx0, ny0, t1, t2 in
          do t1 = d /. (v_1 *. sin(delta_phi))
...
let hybrid main () = () where
  rec () = showupdate (d, delta_phi, radius_alert, radius_protected, 0.0,
