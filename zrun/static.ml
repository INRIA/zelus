...
let rec eval genv env { e_desc; e_loc } =
  ...
  | Eop(op, e_list) ->
     ...
     | Eifthenelse, [e1; e2; e3] ->
        let* v = eval genv env e1 in
        let* v = bool v in
        if v then eval genv env e2
        else eval genv env e3
     | _ -> None 
     end
  | Econstr1 { lname; arg_list } ->
     let* v_list = eval_list genv env arg_list in
     return (Vsconstr1(lname, v_list))
  | Etuple(e_list) ->
     let* v_list = eval_list genv env e_list in
     return (Vstuple(v_list))
  | Eapp(f, e_list) ->
     let* v = eval genv env f in
     let* v_list = eval_list genv env e_list in
  ...
  | Elet({ l_rec; l_eq }, e) ->
     if l_rec then None
       ...
       eval genv (Env.append l_env env) e
  | Ematch { e; handlers } ->
     let* v = eval genv env e in
     match_handlers genv env v handlers
  | Erecord_access { label; arg } ->
     let* arg = eval genv env arg in
     record_access { label; arg }  
  | Erecord(r_list) ->
     let* r_list =
       Opt.map (fun { label; arg } -> let* arg = eval genv env arg in
                                      return ({ label; arg }))
         r_list in
     return (Vrecord(r_list))
  | Erecord_with(r, r_list) ->
     let* r = eval genv env r in
     ...
     return (Vrecord(r))
  | Etypeconstraint(e, _) -> eval genv env e
  | Efun(fe) -> funexp genv env fe
  | Epresent _ -> None
  | Ereset(e_body, e_res) ->
     let* v = eval genv env e_res in
     let* _ = bool v in
     eval genv env e_body
  | Elast _ -> None in
  r
...
and apply env f_args v_list f_body = None
                              
and eval_list genv env e_list =
  match e_list with
  | [] -> return []
  | e :: e_list ->
     let* v = eval genv env e in
  ...
  | EQif(e, eq1, eq2) ->
      ...
      let* v = bool v in
        if v then
          ...
          return env2
  | EQand(eq_list) ->
     let and_eq genv env acc eq =
       let* env_eq = eval_eq genv env eq in
       let* acc = merge env_eq acc in
       return acc in
     let* env_eq = fold (and_eq genv env) Env.empty eq_list in
     return env_eq
  | EQreset(eq, e) -> 
     let* v = eval genv env e in 
     let* v = bool v in
...
and reset genv env eq r =
  if r then eval_eq genv env eq else None
...
and match_handlers genv env v handlers =
  ...
  | { m_pat; m_body } :: handlers ->
     ...
     | Some(env_pat) ->
        ...
        eval genv env m_body

and funexp genv env  fe = return (Vclosure(fe, env))
