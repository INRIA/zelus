...
let check_type_of_main_node name { qualid = qualid; info = { value_typ = tys } }
  ...
  let expected_ty =
    Types.arrow expected_k opt_name expected_ty_arg expected_ty_res in
  try
        ...
        let t1, t2, inp_patt =
          begin match typ_body.t_desc with
            | Tarrow { ty_arg; ty_res } ->
                begin match info.info.value_exp with
                  | Some(Global.Vfun { f_args }) ->
                      let inp_patt = List.hd f_args in
