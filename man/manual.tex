...
values to be accessed at instants of emission.\footnote{This contrasts with 
signals in \esterel{} where values are maintained implicitly and can be 
accessed even in instants when they are not emitted.
The constraint imposed by \zelus{} may sometimes be cumbersome, but it is 
safer, avoids initialization issues, and does not require the allocation of 
a state variable.}

The following program takes two input signals, \zls{x} and \zls{y}, and
returns an integer equal to their sum when both are emitted, the value of 
\zls{x} when it alone is emitted, the value of \zls{y} when it alone is 
emitted, or \zls{0} otherwise.
\begin{chklisting}[withresult]
