...
The \texttt{A} stands for ``any''---the function \texttt{average} can be 
...
Then, we consider two important extensions.
...
\end{chklisting}
The formal parameter \zls{x} of state \zls{Sustain} can be used without 
restriction in the body of the state, and the argument \zls{v} could be 
...
letter are considered to be datatype constructors and cannot be used as 
variable names.}---but generalize it slightly by considering valued events.

%The following function awaits for an event \texttt{e}. When \texttt{e} is 
%present with
%a value \texttt{v}, the next state becomes \texttt{Sustain(v)} with the parameter
%\texttt{x} of state \texttt{Sustain} set to \texttt{v}.
%
As a first step, consider this function that implements the first part of 
...
Turn light on for \texttt{n} seconds and off for \texttt{m} seconds.
\end{quote}
...
The following program integrates the integer sequence \zls{v} and emits 
false until the sum has reached some value \zls{max}. Then, it
emits \zls{true} during \zls{n} instants.
\begin{chklisting}
...
State \zls{S1} defines a local variable \zls{c} which can be used to
compute the weak condition \zls{c = max} and this does not introduce
