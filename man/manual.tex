...
Consider, for instance, the following two simple programs.
\begin{chklisting}[fail]
...
\centering %{-{4
    ...
    \foreach \x in {0,150,300}
    {
        \begin{scope}[xshift=\x]
            ...
            \draw[timesig,thick] (0,0) -- (4,4) node[above right] {time};
        \end{scope}
    }
...
The first program is rejected because \zls{x} is a continuous-time
signal, with $\forall t \in \bR+, x(t) = t$ whereas \zls{->} and
the unit delay \zls{pre} expect a signal evolving at a discrete
time basis. Symetrically, the second program is also meaningless
because \zls{o} should be the discrete-time signal $\forall n \in
\bN, o(n) = n$ and it cannot be integrated to produce \zls{x}.
These two programs
...
keyword \zls{fun}) must be of kind $\AnyKind$; the body of a node
function (declared with keyword \zls{node}) must be of kind
$\NodeKind$ and the body of a continuous-time function (declared with
keyword \zls{hybrid}) must be of kind $\HybridKind$. 
\item
  ...
  $E_2$ must be both combinatorial (with kind \AnyKind); if
  $\AndEq{E_1}{E_2}$ must be discrete (with kind \NodeKind) then both
  $E_1$ and $E_2$ must be discrete. Finally, if $\AndEq{E_1}{E_2}$ is
  expected to be continuous (with kind \HybridKind) then both $E_1$ and
  $E_2$ must be continuous.
  \end{enumerate}
Thus, in a combinatorial function, all sub-expression appearing in
its body must be with kind \AnyKind. Under the definition of a node
(with kind \NodeKind), all sub-expression must be of kind \NodeKind{} or
\AnyKind). Under the definition of an hybrid node (with kind
\HybridKind), all sub-expression must be of kind \AnyKind{} or kind
\HybridKind. A computation with kind \NodeKind{} can be put in parallel
with an expression with kind \HybridKind{} provided it is sampled on a
...
computation is correct. The value of \zls{x} is summed into \zls{o} at 
every
instant \zls{tick} is present. Between those instants, \zls{o} is unchanged.
\begin{chklisting}[withresult]
let hybrid correct(tick) = o where
  ...
  and init o = 0.0
\end{chklisting}
\zls{tick} is of type \zls{zero} which is the type of
    ...
    \begin{tikzpicture}
      [yscale=.5,font=\sffamily\tiny]
        ...
        \draw[ezsig,thick]
            ...
            (2.8,-1.1) cos
            (3.4, 1.0) sin
            (4.0, 2.5) node[above right] {ez};

        \draw [ysig,dashed,very thick]
            (0.0,0.0) -- (0.86,0.0)
            (0.86,0.86) -- (3.20,0.86)
            (3.20,4.06) -- (4.0,4.06)
            ;
...
A zero-crossing event is produced by the operator \zls{up(x)}
which detects that a signal \zls{x} goes from a negative value to a
