...
them in an \ocaml{} module which is then imported into Zélus; see 
\cref{importing}.
...
\begin{tikzpicture}[]
    \path[fill=blue]
        (0,0)
        -- (1.5,0)
        arc (0:120:1.5) coordinate (blue)
        -- cycle;
    \path[fill=red]
        (0,0)
        -- (blue)
        arc (120:240:1.5) coordinate (red)
        -- cycle;
    \path[fill=green]
        (0,0)
        -- (red)
        arc (240:360:1.5) coordinate (red)
        -- cycle;
    \draw[fill=black]
          (-.3,.8)
       -- ++(.6,0)
...
%END IMAGE
%HEVEA\imageflush
\caption{Rotating colored wheel and sensor\label{fig:colorwheel}}
\end{figure}

For example, consider a colored wheel rotating on an axis and for
which we want to compute the rotation direction. This wheel is
composed of three sections with colors blue (\zls{Blue}), red
(\zls{Red}) and green (\zls{Green}).  A sensor observes the
successive colors on the wheel and has to decide if the wheel is
immobile or determine the rotation direction.

We consider that the direction is direct (\zls{Direct}) when there is
a succession of \zls{Red}, \zls{Green}, \zls{Blue}, \zls{Red}...,
the opposite direction being indirect (\zls{Indirect}). There are
some instants where the direction is undetermined
(\zls{Undetermined}) or that the wheel is immobile (\zls{Immobile}).

We program the controller in the following way. First, we introduce two
sum types.  The function \zls{direction} then compares three
successive values of the input stream \zls{i}.
\begin{chklisting}
