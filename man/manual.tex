...
Much the same as for combinatorial functions, the keyword \zls{let} can be 
omitted, for example,
\begin{runverbatim}
let dt = 0.01
...
let distance ((x0,y0), (x1,y1)) =
  ...
  sqrt (d0 *. d0 +. d1 *. d1)
\end{runverbatim}
Note that because \verb-d0- and \verb-d1- denote infinite streams
the computations of \verb+x1 -. x0+ and \verb+y1 -. x1+ are
(virtually) executed in parallel.  Nonetheless, when writing sequences
of definitions \zls{let}/\zls{in} such as above, the sequential order is
preserved for each reaction of the system, that is, the current value
of \zls{d0} is always computed before the current value of
\zls{d1}. The preservation of the sequential order is useful when functions
