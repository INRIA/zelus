...
the value of a stream \zls{c}, which has the value \zls{t} ({\em true}) at 
the first instant, \zls{f} ({\em false}) at the second one, and \zls{t} at 
the
...
unspecified value (denoted here by {\em nil}) at the first instant.
...
\zls{x} & $x_0$ & $x_1$ & $x_2$ & $\cdots$ \\ \hline
\zls{y} & $y_0$ & $y_1$ & $y_2$ & $\cdots$ \\ \hline
\zls{pre x} &  $nil$ & $x_0$ & $x_1$ & $\cdots$ \\ \hline
...
\medskip\noindent{\bf Warning:} A common error is to try to use the 
initialization operator to define the first two values of a
...
definitions of a {\tt node} definition 
(see~\cref{sec:sequential-functions}).
...
(see~\cref{sec:local-defin-mutu}). For example the average function can
...
figure~\cref{half-adder} and it corresponds to the following code:
...
% }-}3
\subsection{Stateful Functions} %{-{3
...
% }-}3
\subsection{Local Definitions and Mutually Recursive Definition} %{-{3
...
% }-}3
\subsection{The Difference between Pre and Last} %{-{3
