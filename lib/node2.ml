...
module Make (SSolver: Zls.STATE_SOLVER) (ZSolver: Zls.ZEROC_SOLVER) =
  struct
	...
	ignore (f_step s (no_time, input)) in
      ...
      let minorstep s input cvec zinvec =
	cstate.major <- false;
	cstate.zoutvec <- no_roots_out;
	cstate.dvec <- ignore_der;
	...
	let reset { state; solver } =
	  ...
	  match solver with
	  | Init -> ()
	  | Running { zstate; sstate; t_start } ->
	     SSolver.reinitialize sstate t_start nvec;
	     ZSolver.reinitialize zstate t_start cvec in
	  ...
	  try
	    ...
	    | Running({ next; sstate; zstate; t_start; t_mesh; t_limit } as s)
		 ...
		 then
		   ...
		   let result = minorstep state input cvec roots in
		   s.output <- result;
		   { time = horizon; status = Interpolate; result = result }
		 else
	    	   match next with
		   | StepRootsFound | StepCascade ->
			...
			then begin s.next <- StepCascade; Cascade end
			  ...
			  s.t_limit <- t_limit;
			  s.next <-
			    if t_limit < stop_time then Integrate
			    else StepStopTimeReached;
			  Horizon(t_limit) in
		      { time = t_mesh; status = status; result = s.output }
		   | Integrate ->
		      log_info "Integrate: time = " t_mesh;
		      let t_nextmesh =
			(* integrate *)
			...
			if has_roots then
			  ...
			  let next =
			    if t = t_limit then StepCascade else Integrate in
			  s.t_mesh <- t;
			  s.next <- next;
			  Success in			    
