...
(* output *)
type return = { time: float; status: status; result: float array; sens: float array array }
...
struct
  ...
  and ('a, 'b) solver_state =
    { zstate: ZSolver.t;
      ...
      (* current time *)
      minput: Sundials.RealArray.t; (* the input is read at discrete-time instants *)
      mutable minput_content: float array; (* to avoid copying minput content every time *)
      mutable output: float array; (* the current output *)
    ...
    let step ({ state; solver } as s) (expected_time, input) =
      try
        ...
        | Init ->
          ...
          let sens = SSolver.arrays_of_sensmat smat in

          let minput = Sundials.RealArray.of_array input in
          let minput_content = Sundials.RealArray.to_array minput in

          let derivative params time cvec dvec =
            derivative state (Sundials.RealArray.to_array params) time cvec dvec in

          let crossing time cvec dvec =
            crossing state minput_content time cvec dvec in

          (* Allocate the solver *)
          let sstate =
            SSolver.initialize derivative minput nvec smat in
        ...
        | Running({ next; sstate; zstate; t_start; t_mesh; t_limit;
          ...
          if expected_time < t_start then
            ...
            let input_change =
              (expected_time > t_time) && not (minput_content = input) in
            ...
            if expected_time <= t_mesh then
              if not input_change then
                ...
                { time = expected_time; status = Interpolate;
                  result = result; sens = SSolver.arrays_of_sensmat smat }
                ...
                s.t_start <- t_time;
                s.t_mesh <- t_time;
                (* copy new input into minput *)
                for i = 0 to Sundials.RealArray.length s.minput do
                  s.minput.{i} <- input.(i)
                done;
                (* copy minput content into minput_content *)
                Sundials.RealArray.into_array s.minput s.minput_content;
                s.output <- result;
                let status =
                  let horizon = horizon state in
                  if horizon = 0.0
                ...
                { time = t_mesh; status = status; result = s.output;
                  sens = SSolver.arrays_of_sensmat smat }
            else
              match next with
              | Integrate ->
                ...
                if t_mesh >= stop_time then
                  begin
                    ...
                    { time = stop_time; status = StopTimeReached;
                      result = s.output; sens = SSolver.arrays_of_sensmat smat }
                  end
                else
