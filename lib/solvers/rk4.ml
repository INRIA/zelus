...
struct (* {{{1 *)
  ...
  *)
  ...
  let init f ydata =
    ...
    p.solver.h <- p.max_step;
    p

  (* Butcher Tableau : (vf http://en.wikipedia.org/wiki/Butcher_tableau )
     0 |
     c | a
     c | a  a
     c | a  a  a
     ...
     b   b  b  b
  *)
  let butcher_c = [|0.0; 1.0/.2.0; 1.0/.2.0; 1.0|]
  ...
  let rec step s time_limit user_y =
    let { stop_time; min_step; abs_tol;
          solver = { sysf = f; y; time = t; h = h;
                     k1 = k1; k2 = k2; k3 = k3; k4 = k4; k = k;
                     y1 = y1 }; } = s in
    ...
    if !debug then log_step t y k1 (t +. h) y1 k4;
  ...
  let get_dky { solver = { last_time = x0; y1 = f0; k1 = f'0;
    ...
    for i = 0 to (length f0 - 1) do
      set y i (px (get f0 i) (get f1 i) (get f'0 i) (get f'1 i))
    done
...
let _ =
  Fixedstep.register
    "rk4F"
    "internal solver: fixed-step Runge-Kutta 4 method"
    (module RungeKutta4 : STATE_SOLVER)
