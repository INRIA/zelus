...
let new_prob () = Zlident.fresh "prob"
...
let typ_prob = Zltypes.make (Deftypes.Tvar)
...
let rec expression prob ({ e_desc = e_desc } as e) =
  ...
  | Eperiod({ p_phase = opt_p1; p_period = p2 }) ->
     { e with e_desc =
		Eperiod({ p_phase =
			    Zlmisc.optional_map (expression prob) opt_p1;
  ...
  | Eapp(app, op, e_list) ->
     ...
     let e_list =
       if Zltypes.is_probabilistic (List.length e_list - 1) op.e_typ then
...
and equation prob ({ eq_desc = desc } as eq) =
  ...
  | EQnext(x, e, e_opt) ->
     let e_opt = Zlmisc.optional_map (expression prob) e_opt in
