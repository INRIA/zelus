...
let new_time () = Zlident.fresh "time"
...
let up major time e =
  let z = Zlident.fresh "z" in
  let ztime = Zlident.fresh "ztime" in
  let env =
...
let rec expression major time ({ e_desc = e_desc } as e) =
  ...
  | Eperiod({ p_phase = opt_p1; p_period = p2 }) ->
     period major time
	    { p_phase = Zlmisc.optional_map (expression major time) opt_p1;
  ...
  | Eapp(app, op, e_list) ->
     ...
     let e_list =
       if Zltypes.is_hybrid (List.length e_list - 1) op.e_typ then
         let head, tail = Zlmisc.firsts e_list in
...
and equation major time ({ eq_desc = desc } as eq) =
  ...
  | EQnext(x, e, e_opt) ->
     let e_opt = Zlmisc.optional_map (expression major time) e_opt in
...
let implementation impl =
  ...
  | Efundecl(n, ({ f_kind = C; f_args = pat_list;
     ...
     let e = expression major time e in
     let head, tail = Zlmisc.firsts pat_list in
