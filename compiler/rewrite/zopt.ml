 ...
 * ren: Zlident.t -> Zlident.t defines the renaming of zero-crossings
 * size: number of entries in env *)
type zenv = { env: Deftypes.tentry Env.t; ren: Zlident.t Env.t; size: int }
 ...
 * zenv.rename: Zlident.t -> Zlident.t, the substitution of zero-crossing variables *)
let rec equation ({ eq_desc = desc } as eq) =
  match desc with
  | EQeq _ | EQpluseq _ | EQder _ | EQinit _ -> eq, zempty
  | EQmatch(total, e, m_h_list) ->
     let m_h_list, zenv =
       Zlmisc.map_fold (fun acc ({ m_body = b } as m_h) ->
...
and equation_list eq_list = 
  Zlmisc.map_fold (fun acc eq -> let eq, zenv = equation eq in
