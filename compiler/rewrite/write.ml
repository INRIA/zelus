  ...
  let eq, defnames, shared_set = match desc with
    ...
    | EQforall ({ for_index = i_list; for_init = init_list;
	 ...
	 | Einit_last(i, e) ->
       ...
       let i_list, xi_out_x = Zlmisc.map_fold index Env.empty i_list in
       let init_list, i_set = Zlmisc.map_fold init S.empty init_list in
...
and expression ({ e_desc = desc } as e) =
  let desc =
    ...
    | Eperiod { p_phase = p1; p_period = p2 } ->
       Eperiod
	 { p_phase = Zlmisc.optional_map expression p1; p_period = expression p2 }
