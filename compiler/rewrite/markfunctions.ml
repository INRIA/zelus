...
let funexp_info { f_args = p_list; f_body = { r_caus } as r } =
  ...
  Causal.mark_and_polarity true r_caus;
  let c_set =
    Causal.vars (List.fold_left Causal.vars Causal.S.empty tc_list) r_caus in
