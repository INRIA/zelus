...
let read ({ eq_write; eq_desc } as eq) =
  let last_acc, acc =
    Vars.fv_eq Zident.S.empty (Zident.S.empty, Zident.S.empty) eq in
  match eq_desc with
  | EQmatch(_, e, _) | EQreset(_, e) ->
      let w = Deftypes.names Zident.S.empty eq_write in
      let last_acc = Zident.S.diff last_acc w in
      let acc = Zident.S.diff acc w in
      Vars.fv Zident.S.empty (last_acc, acc) e
  | _ -> last_acc, acc
...
let rec fresh i eqs =
  ...
  | Before(eqs_list) ->
      let eqs_list, i = Zmisc.map_fold fresh i eqs_list in
      Before(eqs_list), i
  | And(eqs_list) ->
      let eqs_list, i = Zmisc.map_fold fresh i eqs_list in
      And(eqs_list), i
...
let make_read_write xtable itable eq_info_list =
  ...
  let find x table = try Zident.Env.find x table with Not_found -> S.empty in
  ...
  let rec make g (n, eq, w, v, lv) =
    ...
    let l =
      S.remove n
        (Zident.S.fold (fun x iw -> S.union (find x itable) iw) w S.empty) in
    ...
    let l =
      S.remove n
        (Zident.S.fold (fun x iw -> S.union (find x xtable) iw) lv S.empty) in
     let g = Graph.add_before (S.singleton n) l g in
    (* - after an equation [init x = e] where [x in lv] excluding itself *)
    let l =
      S.remove n
        (Zident.S.fold (fun x iw -> S.union (find x itable) iw) lv S.empty) in
    let g = Graph.add_before l (S.singleton n) g in
    g in
...
let make eqs =
  ...
  let xtable, itable, eq_info_list =
    name_to_index (Zident.Env.empty, Zident.Env.empty, []) eqs in
