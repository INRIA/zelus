...
let reset e e_reset =
  ...
  { e with e_desc =
	     Elet({ l_rec = false; l_env = env; l_eq = [eq];
		    l_loc = Zllocation.no_location }, var res e.e_typ) }
...
let rename x renaming =
  ...
  with Not_found ->
    Zlmisc.internal_error "Inline: unbound name" Printer.name x
...
let rec expression renaming ({ e_desc = desc } as e) =
  ...
  | Eapp({ app_inline = i } as app,
     ...
     begin try
	   ...
	   inline (!Zlmisc.inline_all || i) f in
  ...
  | Eperiod { p_phase = p1; p_period = p2 } ->
     { e with e_desc = Eperiod { p_phase = Zlmisc.optional_map (expression renaming) p1;
  ...
  let desc = match desc with
    ...
    | EQnext(x, e, e0_opt) ->
       EQnext(rename x renaming, expression renaming e,
	      Zlmisc.optional_map (expression renaming) e0_opt)
    | EQder(x, e, e0_opt, p_e_list) ->
	 ...
	 { p_cond = scondpat renaming sc;
       ...
       let e0_opt = Zlmisc.optional_map (expression renaming) e0_opt in
    ...
    | EQpresent(p_h_list, b_opt) ->
	 ...
	 { p_cond = scondpat renaming sc;
       ...
       let b_opt =
	 Zlmisc.optional_map (fun b -> let _, b = block renaming b in b) b_opt in
       EQpresent(List.map body p_h_list, b_opt)
    | EQemit(x, e_opt) ->
       EQemit(rename x renaming, Zlmisc.optional_map (expression renaming) e_opt)
    | EQblock(b) ->
    ...
    | EQautomaton(is_weak, s_h_list, se_opt) ->
	 ...
	 List.fold_left build_state_names renaming s_h_list in
       let se_opt = Zlmisc.optional_map (state_exp renaming) se_opt in
...
and letin renaming env p_list e_list e =
  ...
  { e with e_desc =
      Elet({ l_rec = false; l_env = env; l_eq = List.map2 eqmake p_list e_list;
             l_loc = Zllocation.no_location }, expression renaming e) }
...
let implementation_list impl_list = Zlmisc.fold implementation impl_list
