...
let generalise tc =
  ...
  let tc =
    if !Zlmisc.no_simplify_causality_type then tc else simplify_by_io tc in
  (* check_type tc; *)
  gen tc;
  let c_set = vars S.empty tc in
  if not !Zlmisc.no_simplify_causality_type then reduce c_set;
...
let simplify_by_io_env env expected_tc actual_tc =
  let mark_env _ { t_typ = tc; t_last_typ = ltc_opt } =
    mark_and_polarity true tc;
    Zlmisc.optional_unit mark_and_polarity true ltc_opt in
  let simplify_env { t_typ = tc; t_last_typ = ltc_opt } =
    let tc = simplify_by_io tc in
    let ltc_opt = Zlmisc.optional_map simplify_by_io ltc_opt in
  ...
  let cset =
    Env.fold
      (fun _ { t_typ = tc; t_last_typ = ltc_opt } acc ->
         Zlmisc.optional vars (vars acc tc) ltc_opt) env S.empty in
