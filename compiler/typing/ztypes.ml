...
<<<<<<< HEAD:compiler/typing/zltypes.ml
open Zlmisc
open Zlident
=======
open Zmisc
open Zident
>>>>>>> main:compiler/typing/ztypes.ml
open Lident
open Deftypes
open Global
open Modules
...
=======
     let ty_list, acc = Zmisc.map_fold clear acc ty_list in
>>>>>>> main:compiler/typing/ztypes.ml
     product(ty_list), acc
  | Tvec(ty_arg, size) ->
...
=======
	      if Zident.compare n1 n2 = 0 then unify ty_res1 ty_res2
	      else
		let m = Zident.fresh (Zident.source n1) in
>>>>>>> main:compiler/typing/ztypes.ml
		let ty_res1 =
		  subst_in_type
...
=======
  | Tname(n1), Tname(n2) when Zident.compare n1 n2 = 0 -> ()
>>>>>>> main:compiler/typing/ztypes.ml
