...
module LANG =
  struct
    ...
    let not_in tl =
      (* Returns e if is not in l, or next e if it is. Iterated with Zlmisc.fix,
    ...
    let rec inject p =
      ...
      and find_record_type_fields typ =
        let { t_desc = desc } = Zltypes.typ_repr typ in
        match desc with
      ...
      match p.p_desc with
        ...
        | Econstr0pat s ->
            let variants = 
              let { t_desc = desc } = Zltypes.typ_repr p.p_typ in
              match desc with
...
let check_match_handlers loc match_handlers =
  ...
  Zlmisc.optional_unit partial_matching loc r.C.not_matched;
