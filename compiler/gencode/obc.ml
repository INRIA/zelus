...
type exp = 
  ...
  | Olocal of Zlident.t (* read of local value *)
  | Ovar of is_mutable * Zlident.t (* read of local variable *)
...
and inst =
  ...
  | Oletvar of Zlident.t * is_mutable * Deftypes.typ * exp option * inst
  | Ofor of bool * Zlident.t * exp * exp * inst
  | Owhile of exp * inst
...
and left_state_value =
  ...
  | Oleft_instance_name of Zlident.t
  | Oleft_state_name of Zlident.t
...
and pattern = 
  ...
  | Ovarpat of Zlident.t * type_expression
  | Oconstpat of immediate
  | Oaliaspat of pattern * Zlident.t
...
and method_call =
  { met_machine: Lident.t option; (* the class of the method *)
    ...
    met_instance: (Zlident.t * exp path) option;
...
and type_expression = 
  | Otypevar of string
  | Otypefun of kind * Zlident.t option * type_expression * type_expression
