...
type env = entry Env.t (* the symbol table *)
 ...
 and sort =
   ...
   | Out of Zlident.t * Deftypes.tsort
   (* the variable [x] is stored into [y.(i_1)...(i_n); e.g. [x out y]] *)

 and loop_path = Zlident.t list
...
let fprint ff (env: entry Env.t) =
  ...
  Zlident.Env.fprint_t fprint_entry ff env
...
let entry_of n env =
  try
    Env.find n env
  with Not_found ->
    Zlmisc.internal_error "Unbound variable" Printer.name n
...
let pluseq ({ e_sort = sort; e_size = ei_list } as entry)
  ...
  let ln =
    ...
    | Out(n, sort) ->
       ...
       | _ -> Zlmisc.internal_error "Unbound variable" Printer.name n in
  { code with step =
...
let rec exp env loop_path code { Zelus.e_desc = desc } =
  ...
  | Zelus.Econstr1(ln, e_list) ->
      let e_list, code = Zlmisc.map_fold (exp env loop_path) code e_list in
  ...
  | Zelus.Eop(Zelus.Eup, [e]) ->
     ...
     let e = if !Zlmisc.zsign then Zaux.sgn e else e in 
  ...
  | Zelus.Eop(Zelus.Eupdate, [e1; i; e2]) ->
     let _, se = Zltypes.filter_vec e1.Zelus.e_typ in
  ...
  | Zelus.Eop(Zelus.Econcat, [e1; e2]) ->
     let _, s1 = Zltypes.filter_vec e1.Zelus.e_typ in
     let _, s2 = Zltypes.filter_vec e2.Zelus.e_typ in
  ...
  | Zelus.Eapp(_, e_fun, e_list) ->
     ...
     let se_list, ne_list, ty_res =
       Zltypes.split_arguments e_fun.Zelus.e_typ e_list in
     let e_fun, code = exp env loop_path code e_fun in
     let se_list, code = Zlmisc.map_fold (exp env loop_path) code se_list in
     let ne_list, code = Zlmisc.map_fold (exp env loop_path) code ne_list in
     let e_fun = app e_fun se_list in
     match ne_list with
     | [] -> e_fun, code
     | _ -> let k = Zltypes.kind_of_funtype ty_res in
...
let rec equation env loop_path { Zelus.eq_desc = desc } code =
  ...
  | Zelus.EQforall { Zelus.for_index = i_list; Zelus.for_init = init_list;
	 ...
	 { mem = m; instances = j; init = i; reset = r; step = s } =
       Zlmisc.map_fold init code init_list in
...
and match_handlers env loop_path p_h_list =
  ...
  Zlmisc.map_fold body empty_code p_h_list
...
let machine n k pat_list { mem = m; instances = j; reset = r; step = s }
  ...
  | Deftypes.Tdiscrete(true) | Deftypes.Tcont | Deftypes.Tproba ->
    ...
    let pat_list, p = Zlmisc.firsts pat_list in
    let body =
       { ma_kind = k;
	 ma_params = pat_list;
	 ma_initialize = None;
