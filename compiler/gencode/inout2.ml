 ...
 *- let dzero s i = for i = i to s - 1 do !dvec.(i) <- 0.0 done *)

(* Only the method "step" is changed.
 *- suppose that [s] is the length of the state vector;
 *- x1,..., xn: sn are the continuous state variables with sizes si;
...
let machine f ({ ma_memories = m_list; ma_methods = method_list } as mach) =
  ...
  try
    ...
    { mach with ma_methods = { mdesc with me_body = body } :: method_list }
  with
