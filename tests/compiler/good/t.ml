      ...
      let (m_20:(int, int) Ztypes.node) =
          let machine_30  = 
            ...
            let machine_30_reset self  =
              ((self.m_28 <- 0):unit) in
            let machine_30_step self ((m_21:int)) =
              ((self.m_28 <- self.m_21; self.m_28):int) in
            Node { alloc = machine_30_alloc; reset = machine_30_reset;
                                             step = machine_30_step } in
            machine_30 in
      m_20
let (f) = let (m_22:(int, int) Ztypes.node) = r_9 in
          m_22
let (r_12) =
      let (m_23:(int, int) Ztypes.node) =
          let machine_32  = 
            let Node { alloc = i_31_alloc; step = i_31_step;
                                           reset = i_31_reset } = f  in
            let machine_32_alloc _ =
              ();
              { m_24 = (42:int); m_29 = (42:int); m_25 = (42:int);
                i_31 = i_31_alloc () (* discrete *)  } in
            let machine_32_reset self  =
              ((self.m_29 <- 0; i_31_reset self.i_31 ):unit) in
            let machine_32_step self ((m_24:int)) =
            ...
            machine_32 in
      m_23
let (g) = let (m_26:(int, int) Ztypes.node) = r_12 in
          m_26
