...
type 'a random_var = RV : ('b, 'a) Infer_ds_ll.ds_node -> 'a random_var
  ...
  let is prob =
    begin match affine_of_expr mu with
      ...
      | Some (AErvar (m, RV x, b)) ->
          begin match Infer_ds_ll.get_distr_kind x with
            | KGaussian ->
                let rv =
                  Infer_ds_ll.assume_conditional x (AffineMeanGaussian(m, b, std))
                in
                Some { value = (Ervar (RV rv)) }
            | _ -> None
          end
...
let beta(a, b) =
  ...
  let is prob =
    Some {value = Ervar (RV (Infer_ds_ll.assume_constant (MBeta (a, b)))) }
  in
...
let bernoulli p =
  ...
  let with_beta_prior f =
    begin match p.value with
      | Ervar (RV par) ->
          begin match Infer_ds_ll.get_distr_kind par with
            | KBeta -> Some (f (RV par))
            | _ -> None
          end
      | _ -> None
  ...
  let is prob =
    with_beta_prior
      (fun (RV par) ->
         { value = Ervar (RV (Infer_ds_ll.assume_conditional par CBernoulli)) })
  in
  let iobs (prob, obs) =
    with_beta_prior
      (fun (RV par) -> Infer_ds_ll.observe_conditional prob par CBernoulli obs)
  in
  ds_distr_with_fallback d is iobs
...
let rec expr_dist_of_expr : type a. a expr -> a expr_dist =
  fun expr ->
    begin match expr.value with
      | Econst c -> EDconst c
      | Ervar (RV x) -> EDdistr (Infer_ds_ll.get_distr x)
